...
=================================
Stores link codes and sessions in MySQL so the Discord bot and Railway server can communicate.
"""
...
try:
    ...
    class DatabaseManager:
        """Database manager for standalone deployment."""
        ...
        def get_unread_mail(self, user_id):
            return self._execute("SELECT * FROM mail WHERE user_id = %s AND is_read = 0 ORDER BY created_at DESC", (user_id,), fetch_all=True)
        
        def get_all_mail(self, user_id):
            return self._execute("SELECT * FROM mail WHERE user_id = %s ORDER BY created_at DESC LIMIT 50", (user_id,), fetch_all=True)
        ...
        def delete_mail(self, mail_id, user_id):
            result = self._execute("DELETE FROM mail WHERE mail_id = %s AND user_id = %s", (mail_id, user_id))
        ...
        def get_transaction_history(self, user_id, limit=20):
            return self._execute("SELECT * FROM transactions WHERE user_id = %s ORDER BY created_at DESC LIMIT %s", (user_id, limit), fetch_all=True)
        
        def get_timeline_entries(self, category=None, limit=50):
            return self._execute("SELECT * FROM timeline ORDER BY event_date DESC LIMIT %s", (limit,), fetch_all=True)
        
        def get_all_players(self, limit=100):
            return self._execute("SELECT user_id, username, drachma, god_parent FROM players ORDER BY drachma DESC LIMIT %s", (limit,), fetch_all=True)
        
        # ═══════════════════════════════════════════════════════════════════════
        # LINK CODE METHODS (stored in database)
        # ═══════════════════════════════════════════════════════════════════════
        
        def store_link_code(self, code, discord_id, discord_username, expires_at):
            """Store a link code in the database."""
            self._execute("DELETE FROM web_link_codes WHERE discord_id = %s", (discord_id,))
            return self._execute(
                "INSERT INTO web_link_codes (code, discord_id, discord_username, expires_at) VALUES (%s, %s, %s, %s)",
                (code, discord_id, discord_username, expires_at)
            )
        
        def get_link_code(self, code):
            """Get a link code from the database."""
            return self._execute(
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # SESSION METHODS (stored in database)
        # ═══════════════════════════════════════════════════════════════════════
...
class CampHalfBloodServer:
    def __init__(self, host='0.0.0.0', port=None, static_dir=None):
        self.host = host
    ...
    async def _api_check_session(self, request):
        session = self._get_session(request)
        if session:
            return web.json_response({'authenticated': True, 'discord_id': session['discord_id'], 'discord_username': session['discord_username']})
    ...
    async def _api_get_profile(self, request):
        ...
        return web.json_response({
            ...
            'cabin': {'cabin_id': cabin['cabin_id'], 'cabin_name': cabin['cabin_name'], 'divine_favor': cabin.get('divine_favor', 0)} if cabin else None,
    ...
    async def _api_get_public_timeline(self, request):
        if not db:
